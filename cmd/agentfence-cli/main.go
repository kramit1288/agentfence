...
func listApprovals(args []string) error {
	service, err := approvalServiceFromFlags("list-approvals", args)
	if err != nil {
		return err
	}
...
func resolveApproval(args []string, approve bool) error {
	...
	service := approval.NewService(approval.NewFileRepository(storePath))
...
func approvalServiceFromFlags(name string, args []string) (*approval.Service, error) {
	var storePath string
	flags := flag.NewFlagSet(name, flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	flags.StringVar(&storePath, "store", approvalStorePath(), "path to the approval store JSON file")
	...
	return approval.NewService(approval.NewFileRepository(storePath)), nil
}
